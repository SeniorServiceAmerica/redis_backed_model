...
describe RedisBackedModel::SortedSet do
  ...
  describe "#to_instance_variable_name" do
    ...
    let(:attributes) { {'score_[foo|bar]' => '[foo_id|bar_score]'} }
    let(:sorted_set) { RedisBackedModel::SortedSet.new(redis_backed_model, {'score_[foo|bar]' => '[foo_id|bar_score]'}) }
    let(:name_as_string) { sorted_set.to_instance_variable_name.to_s.deinstance_variableize }
